...
def Path(posi, orie, pathPoly, moveVect, moveMagn, spee, offs, use_GrouCast, use_PosiAdju, use_Orie, use_OrieCons, scen, math, mathutils, Math, pathObje, debu = False):
		...
		for a in range(len(sameList)):
			...
			if sameList[a] == False and (edg_List[a] in edgeExcl) == False:
				...
				if use_Prop:
					...
					pol1, pol2 = path_geom.EdgeBordList(inde = edg_List[a])
				if pol1 != pathPoly:
	...
	#if use_GrouCast: loca = GrouCast(ver1, norm, loca, pathPoly, Math)
	
	# orient the object to match the normal of the polygon the object is over
	if use_Orie: orie = Orie(ver1, norm, orie, loca, Math, use_OrieCons, math, mathutils)

	return loca, orie, pathPoly

# TODO: moving on a rotated incline changes direction of movement
def PathGrou(posi, pathPoly, scen, math, mathutils, Math, pathObje, debu = False):
	# TODO:
	...
	# loca was at one point across one boundary, but not the other edge of the same polygon, causing pathList to be empty. what caused loca to be outside path boundary?
	# make one function consistent with all scenarios
	loca = posi
	pathList = [pathPoly]
	# index of the polygon the object is over
	edgeExcl = []
	while len(pathList) > 0:
		# TODO: combine into one function. consider making a new class
		use_Prop = scen.objects[scen.name + "." + pathObje]["use_Prop"]
		if use_Prop:
			# read current polygon geometry
			edg1, edg2, edg3, ver1, ver2, ver3, nor1, nor2, nor3, pol1, pol2, pol3 = PolyGeom(scen, pathList[0], pathObje)
			refeList = [EdgeRefeList(scen, edg1, pathObje), EdgeRefeList(scen, edg2, pathObje), EdgeRefeList(scen, edg3, pathObje)]
			norm = NormList(scen, pathList[0], pathObje)
		else:
			# read path geometry. (path geometry is stored to blender properties if use_Prop == True)
			import path_geom
			# read current polygon geometry
			edg1, edg2, edg3, ver1, ver2, ver3, nor1, nor2, nor3, pol1, pol2, pol3 = PolyGeomScri(scen, pathList[0], pathObje, path_geom)
			refeList = [path_geom.EdgeRefeList(inde = edg1), path_geom.EdgeRefeList(inde = edg2), path_geom.EdgeRefeList(inde = edg3)]
			norm = path_geom.NormList(inde = pathList[0])
		# is the object still within the triangle
		sam1 = Same(ver1, nor1, loca, pathList[0], pol1, Math)
		sam2 = Same(ver2, nor2, loca, pathList[0], pol2, Math)
		sam3 = Same(ver3, nor3, loca, pathList[0], pol3, Math)
		sameList = [sam1, sam2, sam3]
		edg_List = [edg1, edg2, edg3]
		ver_List = [ver1, ver2, ver3]
		nor_List = [nor1, nor2, nor3]
		if (False in sameList) == False:
			pathPoly = pathList[0]
			break
		else:
			for a in range(len(sameList)):
				# the object is inside a new polygon. decide next action if the edge is not in the edgeExcl list
				if sameList[a] == False and (edg_List[a] in edgeExcl) == False:
					# get the next poly, and add it to the pathList
					if use_Prop: pol1, pol2 = EdgeBordList(scen, edg_List[a], pathObje)
					else: pol1, pol2 = path_geom.EdgeBordList(inde = edg_List[a])
					if pol1 != pathList[0]:
						pathList.append(pol1)
						edgeExcl.append(edg_List[a])
					else:
						# if pol2 is a border object
						if pol2 != -1:
							pathList.append(pol2)
							edgeExcl.append(edg_List[a])
		pathList.pop(0)
	# update the location of the object to lie on the plane of the polygon
	# TODO: adjust final position to adhere to speed. try drawing a vector from previous to new and scaling. what about when the object crosses a polygon?
	loca = GrouCast(ver1, norm, loca, pathPoly, Math)
	return loca, pathPoly
